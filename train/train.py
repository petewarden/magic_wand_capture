...
def build_cnn(seq_length):
  ...
  model = tf.keras.Sequential([
      ...
      tf.keras.layers.Conv2D(16, (4, 1), padding="same",
                             activation="relu",
                             kernel_initializer=tf.keras.initializers.TruncatedNormal),  # (batch, 42, 1, 16)
      tf.keras.layers.MaxPool2D((3, 1), padding="same"),  # (batch, 14, 1, 16)
      tf.keras.layers.Dropout(0.1),  # (batch, 14, 1, 16)
      tf.keras.layers.Flatten(),  # (batch, 224)
      tf.keras.layers.Dense(16, activation="relu",
                            kernel_initializer=tf.keras.initializers.TruncatedNormal),  # (batch, 16)
      tf.keras.layers.Dropout(0.1),  # (batch, 16)
      tf.keras.layers.Dense(4, activation="softmax",
      kernel_initializer=tf.keras.initializers.TruncatedNormal)  # (batch, 4)
  ])
...
if __name__ == "__main__":
  ...
  parser.add_argument("--data_source", "-d")
  parser.add_argument("--epochs", "-e", type=int, default=50)
  parser.add_argument("--steps_per_epoch", "-s", type=int, default=1000)
  ...
  elif args.data_source == "v1":
    train_len, train_data, valid_len, valid_data, test_len, test_data = \
        load_data("./data/train", "./data/valid", "./data/test", seq_length)
  else:
    train_len, train_data, valid_len, valid_data, test_len, test_data = \
        load_data("gesture_data_v2/training_data.json",
        "gesture_data_v2/validation_data.json",
        "gesture_data_v2/testing_data.json", seq_length)
  ...
  train_net(model, model_path, train_len, train_data, valid_len, valid_data,
            test_len, test_data, args.model, args.epochs, args.steps_per_epoch,
            args.batch_size)
